...
k <- 1
for (i in 1:nlevels(gono.data$ant)) {
...
# labs compared to mean
postscript(file = "resultaten/figure 3.eps", width = 7, height = 7)
par(mar = c(4.5, 4.5, 0.5, 0.5), yaxs = "i"); plot.new(); plot.window(xlim = c(-2, 2), c(n.lab+1, 0))
abline(h = 1:n.lab, col = 8, lty = 3)
box(); axis(1, at = seq(-2, 2, 1), labels = 2^seq(-2, 2, 1)); axis(2, at = 1:n.lab, labels = lab.data$lab); abline(v = 0, lty = 2)
...
# refmics
postscript(file = "resultaten/figure 4.eps", width = 7, height = 7*sqrt(2))
